...
var start = moment("2014-09-01 12:00 +0200", "YYYY-MM-DD HH:mm Z");

if(arrival.isAfter(moment())) {
    ...
    // Countdown finished
    d3.xml("img/index_finished.svg", "image/svg+xml", function (xml) {
        // Append imported SVG to DOM
        document.body.appendChild(xml.documentElement);
        ...
        var diff = moment.duration(arrival.diff(start));
        d3.select("#days").text(Math.ceil(diff.asDays()) + " days");
    });
}